...
// 测试双向实时转换交互模式
func TestRealtimeBidirectionalConversion(t *testing.T) {
	testCases := []struct {
		name          string
		initialHTML   string
		packagePrefix string
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			goCode, err := simulateHTMLToGoConversion(server.URL, htmlInput, tc.packagePrefix)
			if err != nil {
			...
			html, err := simulateGoToHTMLConversion(server.URL, modifiedGoCode)
			if err != nil {
				t.Fatalf("Go到HTML转换失败: %v", err)
			}
			...
			finalGoCode, err := simulateHTMLToGoConversion(server.URL, modifiedHTML, tc.packagePrefix)
			if err != nil {
...
// 模拟HTML到Go的转换请求
func simulateHTMLToGoConversion(serverURL, html, packagePrefix string) (string, error) {
	reqBody := ConvertRequest{
		HTML:          html,
...
// 模拟Go到HTML的转换请求
func simulateGoToHTMLConversion(serverURL, goCode string) (string, error) {
	reqBody := ConvertRequest{
		GoCode:    goCode,
		Direction: "go2html",
	}
